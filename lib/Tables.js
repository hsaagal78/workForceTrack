...
// Add a new employee to the database
  ...
  updateEmployeeRole() {
    return new Promise((resolve, reject) => {
      //
      const getRoleIdSql = 'SELECT id FROM roles WHERE title = ?';
     
      const titleWithoutQuotes = this.title.trim(); 
      this.connection.query(getRoleIdSql, titleWithoutQuotes, (err, results) => {
        if (err) {
          console.log('Error:', err);
          reject(err);
        } else {
          ...
          if (results.length === 0) {
            ...
            this.connection.query(updateSql, [roleId, this.first_name, this.last_name], (err, res) => {
              if (err) {
                console.log('Error:', err);
                reject(err);
              } else {
                console.log('Employee change role successfully!');
                resolve();
              }
            });
          }
